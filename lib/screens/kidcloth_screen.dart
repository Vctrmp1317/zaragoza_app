...
class _listProductsState extends State<kidProducts1> {
  ...
  Future refresh() async {
    ttsNinio.setSpeechRate(0.5);
    ttsNinio.speak('Estás en la sección de niños.');
      ...
      onLongPressEnd: (details) {
        ...
        if (text.contains('ir a') || text.contains('buscar')) {
          ...
          if (ind > articulos.length) {
            ttsNinio.speak('No hay mas articulos');
          } else
            ttsNinio.speak(articulos[ind].modelo! +
                '. color:' +
                articulos[ind].color! +
                ', marca:' +
                articulos[ind].marca! +
          ...
          setState(() {
            idArticuloNinio = articulos[ind].id!;
            articuloService.addVistaArticulo(idArticuloNinio);
